    ...
    # ==================== FEATURE SELECTION ====================
        ...
        if st.button("🚀 Calculate Feature Importance", type="primary", use_container_width=True):
            with st.spinner('Calculating...'):
                ...
                st.session_state.feature_importance = pd.DataFrame({'Feature': rf_series.index, 'Importance': rf_series.values})
                st.session_state.mutual_info = pd.DataFrame({'Feature': mi_series.index, 'Mutual_Info': mi_series.values})
                st.session_state.chi2_scores = pd.DataFrame({'Feature': chi2_series.index, 'Chi2_Score': chi2_series.values})
