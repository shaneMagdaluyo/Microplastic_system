...
def main():
    ...
    # ==================== MODEL EVALUATION (CLEAN TABLES + ENHANCED HEATMAPS) ====================
        ...
        if evaluation_results and len(evaluation_results) > 0:
            ...
            with col2:
                ...
                st.metric("📊 Avg Accuracy", f"{metrics_df['Accuracy'].mean():.3f}")
            ...
            if selected_model:
                ...
                if cm is not None and cm.size > 0:
                    ...
                    # Create professional heatmap with count AND percentage
                    annotations = []
                    ...
                    # Add green boxes around diagonal (correct predictions)
                    shapes = []
                    for i in range(n_classes):
                        shapes.append(dict(
                            type="rect",
                    ...
                    fig_cm.update_layout(
                        title=dict(
                            text=f"<b>{selected_model}</b><br><sub>Confusion Matrix (Count & Percentage)</sub>",
                        ...
                        paper_bgcolor='white',
                        xaxis=dict(
                            title="<b>Predicted Label</b>",
                            tickfont=dict(size=12)
                        ),
                        yaxis=dict(
                            title="<b>True Label</b>",
                            tickfont=dict(size=12)
                        ),
                    ...
                    with st.expander("📋 View Per-Class Metrics", expanded=False):
            ...
            for i, row in enumerate(ranked_df.itertuples(), 1):
                ...
                st.markdown(
                    ...
                    f"F1 Score: <code>{row._3:.3f}</code>",
                    unsafe_allow_html=True
                )
    ...
    # Section: Feature Importance
        ...
        if st.session_state.feature_importance is not None:
            ...
            with col1:
                ...
                st.dataframe(
                    display_df[['feature', 'importance', 'percentage']],
                    column_config={
